...
void scanner_faces::setup() {
	...
	//
	// Lo paso a 1280x720 (mi pantalla) con esc_HD2Mon
	//
...
void scanner_faces::update_haarFinder() {
	...
	if(finder.size()>0) {
		...
		rectHaar.x = faceRect_Tgt.x+faceRect_Tgt.width/2-faceRect_Tgt.width/2;	// tiembla mucho
...
//                fboFT.draw(0,0);
        ...
        if(scnAct==SCN_PRESCAN) {
            ...
            if(guidesEyes) {
                ...
                draw_featEnMarco(leftEye,0, true, true);	// (FT_feat, gap (antes 15), uu, rr)
                draw_featEnMarco(rightEye,0, true, true);
