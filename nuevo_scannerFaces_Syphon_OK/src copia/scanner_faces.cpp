...
void scanner_faces::setupFotos() {
    // SAlen con Proporcion W/H 4/3
    float proporWH = 4.0/3.0;
...
//	focusPts.push_back(ofPoint(ofGetWidth()*4/5, ofGetHeight()/2));
	...
	rectHaar.width = 768;//480;//400;
	rectHaar.height = 1024;//640;//rectHaar.width/640*480;
	rectHaar.x = (1024-768)/2;
	rectHaar.y = 0;//(480-rectHaar.height)/2;
//	float hhh = hCam;
//	float www = hhh/proporWH;
...
void scanner_faces::draw() {
    ...
    camSyphon.draw(0,0, 1,1);   // Cagate lorito. Si no se pone esto, no dibuja camImage
    camImage.draw(0,0);
