...
void scanner_faces::update_haarFinder() {
	...
	if(finder.size()>0) {
		...
		//
		float dyUp  = 0;//3*faceRect.height/10.0;	//
		float dyDwn = 0;//2*faceRect.height/10.0;	//
...
//		ofDisableAlphaBlending();
		...
		// TEST
//		mensajesT[idMsgShow].draw(fontMensajes, fontMensajes_peque);
//		mensajesT[idMsgShow].drawImage(rectMensajes);
