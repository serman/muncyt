...
void ofxBlobTracker::setup(int kindOfBackgroundS){
    kindOfBG=kindOfBackgroundS;
    IDCounter = 0;
	numEnter = 0;
	numLeave = 0;
	nSize = 0;
    ...
    //bg_model.bShadowDetection = true;
    //bg_model.varThreshold=30;
    // bg_model.nmixtures = 3;
    //bgs.nextProcessor();
    
    //moog normal
    //    bg_model.backgroundRatio=.1;
    openCVInputImage.allocate(VIDEOWITH,VIDEOHEIGHT);
    floatBgImg.allocate(VIDEOWITH,VIDEOHEIGHT);
    
    if(kindOfBG==OPENCV){
        bg_modelMOG=new cv::BackgroundSubtractorMOG2(20,16,true);
        //parameters=http://docs.opencv.org/modules/video/doc/motion_analysis_and_object_tracking.html
        bg_modelMOG->nmixtures=4;
        bg_modelMOG->backgroundRatio=0.7;
        bg_modelMOG->varThresholdGen=49;
        bg_modelMOG->fTau=0.8; //Shadow threshold. a mas alto mas sombra detecta
    }else if(kindOfBG==OFXCV){
        bg_modelOFX= new ofxCv::RunningBackground();
        bg_modelOFX->setLearningTime(40);
        bg_modelOFX->setThresholdValue(40);
        bg_modelOFX->setDifferenceMode(ofxCv::RunningBackground::DARKER);
    }else if(kindOfBG==CUSTOM){

    }
}


void ofxBlobTracker::setFiltersParam(int _learnRate,int _smooth ){
    learnRate=_learnRate;
    if(kindOfBG==OPENCV){
        bg_modelMOG->history=learnRate;
    }else if(kindOfBG==OFXCV){
        bg_modelOFX->setLearningTime(learnRate);
    }else if(kindOfBG==CUSTOM){
        float fLearnRateDividido=(float)learnRate/10000;

    }
    if(_smooth!=0)	bSmooth=true;
    else bSmooth=false;
    smooth=_smooth;
}

//actualiza la imagen de foreground y de background
void ofxBlobTracker::update1( cv::Mat &image_original, int _threshold ){
    ...
    if(kindOfBG==OPENCV){
        ...
        bg_modelMOG->operator()(image_original,fore,learnRate/10000.0);
        ofxCv::threshold(fore, fore, 127);
       // bg_modelMOG->getBackgroundImage(backgr);
        ...
        bg_modelOFX->setThresholdValue(_threshold);
        bg_modelOFX->update(image_original,fore);
     //   backgr=bg_modelOFX->getBackground();
    }else if(kindOfBG==CUSTOM){
...
void ofxBlobTracker::update2( int _threshold, int _minArea,int _maxArea , int _nConsidered , double _hullPress , bool _bFindHoles , bool _bUseApproximation ){
   ...
   // cv::Mat tmpCvImgage;
    ...
    //tmpCvImgage=fore;
 //   *openCVInputImage.getCvImage() = fore.operator IplImage();
    
    ofxCv::toOf(fore, tempOfImage);
    openCVInputImage.ofxCvImage::setFromPixels(tempOfImage.getPixelsRef());
    
    if(kindOfBG==CUSTOM){
        if (_threshold != -1)
            openCVInputImage.threshold(_threshold);
    }    
    openCVInputImage.updateTexture();
    
    contourFinder.findContours(openCVInputImage, _minArea, _maxArea, _nConsidered, _hullPress, _bFindHoles, _bUseApproximation);
    track(&contourFinder);
}
// Esta funcion trabaja con la version primera del bg substraction
void ofxBlobTracker::updateBG(ofxCvGrayscaleImage &sourceGrayImage){
    ...
    backgroundImage= floatBgImg; //convertimos a la imagen a grises
    backgroundImage.flagImageChanged();
    cvSub(backgroundImage.getCvImage(),sourceGrayImage.getCvImage(), openCVInputImage.getCvImage());
    
    fore=ofxCv::toCv(openCVInputImage);
    
    if(bResetBg==true){
        floatBgImg=sourceGrayImage;
        backgroundImage=sourceGrayImage;
        bResetBg=false;
    }
}
void ofxBlobTracker::resetBG(){
    bResetBg=true;
}

void ofxBlobTracker::drawDebug(){
    ofxCv::drawMat(fore,500,0,480,360);
    ofxCv::drawMat(backgr,500,400,480,360);
}


void ofxBlobTracker::setBg(ofxCvGrayscaleImage &newbg){
...
void ofxBlobTracker::draw( float _x, float _y, float _width, float _height ) {
    ...
    for( int i=0; i<(int)trackedBlobs.size(); i++ ) {
     ...
     /*           ofFill();
        int posx=trackedBlobs[i].boundingRect.x*500;
        int posy=trackedBlobs[i].boundingRect.y*400;
        ofRect(posx,posy,10,10);*/
