/*
 *  ofxBlobTracking.h
 *
 *  Created by Ramsin Khoshabeh on 5/4/08.
 *  Copyright 2008 risenparadigm. All rights reserved.
 *
 * Changelog:
 * 08/15/08 -- Fixed a major bug in the track algorithm
 */

#ifndef OFXBLOBTRACKER
#define OFXBLOBTRACKER

#include "ofxBlob.h"
#include "ofxContourFinder.h"
#include "ofxCv.h"
#include "ofxBGS.h"
class ofxBlobTracker{
public:
    enum {CUSTOM,OPENCV,OFXCV};
	ofxBlobTracker();
    void setup(int kindOfBackgroundS);

    void    draw( float _x = 0, float _y = 0, float _width = 0, float _height = 0);

    void 	setBg(ofxCvGrayscaleImage& newbg);
        
    int     size(){return trackedBlobs.size(); };
    
    ofxBlob operator[](unsigned int _n){ if ( (_n >= 0U) && (_n < trackedBlobs.size()) ) return trackedBlobs[_n]; };
    
    ofEvent<ofxBlob>    blobAdded;
    ofEvent<ofxBlob>    blobMoved;
    ofEvent<ofxBlob>    blobDeleted;
    
    int     movementFiltering;
    bool    bUpdateBackground;
	vector<ofxBlob> trackedBlobs;		//tracked blobs
    void setFiltersParam(int _highpassAmp,int _smooth );
    void update1( cv::Mat &image_original, int _threshold = -1 );
    void update2( int _threshold = -1,
                int minArea = 20 ,int maxArea = (340*240)/3, int nConsidered = 10,
                double hullPress = 30, bool bFindHoles = false, bool bUseApproximation = true );
    ofxBGS bgs;
        void drawDebug();
    
    void resetBG();
    ofxCvShortImage		floatBgImg;
    ofxCv::Mat fore;
    ofxCv::Mat backgr;
    ofxCvGrayscaleImage openCVInputImage;
    int kindOfBG;
    void updateBG(ofxCvGrayscaleImage &sourceGrayImage);
    float fLearnRateDividido;
    float learnRate;
private:
    bool bSmooth, bHighpass, bMiniMode, bAmplify=false;
    int smooth=2;
    int highpassBlur=2;
    int highpassAmp= 5;
    
	void    track(ofxContourFinder* newBlobs);
	int     trackKnn(ofxContourFinder *newBlobs, ofxBlob *track, int k, double thresh);
    
    ofxContourFinder    contourFinder;
    ofxCvGrayscaleImage backgroundImage;
    ofxCvGrayscaleImage subtractBg, highpassImg, ampImg;
	//vector<ofxBlob> trackedBlobs;		//tracked blobs
    
	int             IDCounter, numEnter, numLeave, nSize;
    int             width, height;
    bool bResetBg;
    cv::BackgroundSubtractorMOG2 *bg_modelMOG;
    ofxCv::RunningBackground *bg_modelOFX;
   // ofxCv::RunningBackground *bg_modelCUSTM;
    
ofImage tempOfImage;


};

#endif
